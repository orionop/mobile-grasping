...
def main():
    ...
    ax.plot(to, eo, label="Open-loop", color="#c23b3b", lw=1.8)
    ax.plot(tc, ec, label="Closed-loop (reactive QP)", color="#2b6cb0", lw=1.8)
    ax.axvline(drive_start, color="gray", ls=":", lw=1,
               label="base motion onset (0.10 m/s)")
    ax.axhline(1.0, color="green", ls="--", lw=0.8, label="1 cm grasp tolerance")
    ax.set_xlabel("Time (s)")
    ax.set_ylabel("End-effector position error (cm)")
    ax.set_title("End-effector tracking during base motion: open-loop vs. closed-loop")
